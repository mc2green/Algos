...
//string separated by a single space. Each string will contain the count of each race on the side of good and evil.
...
function goodVsEvil(good, evil){
    const goodArrMultiples = [1,2,3,3,4,10];
    const evilArrMultiples = [1,2,2,2,3,5,10]
    const goodArrOfStrings = good.split(' ')
    const evilArrOfStrings = evil.split(' ')
    const goodArrOfNumbers = goodArrOfStrings.map((str) => (parseInt(str, 10)))
    const evilArrOfNumbers = evilArrOfStrings.map((str) => (parseInt(str, 10)))
    let goodSum = 0;
    let evilSum = 0;
    for (let i = 0; i < goodArrOfNumbers.length; i++) {
      goodSum += goodArrOfNumbers[i] * goodArrMultiples[i]
  
    }
    for(let j = 0; j < evilArrOfNumbers.length; j++){
      evilSum += evilArrOfNumbers[j] * evilArrMultiples[j]
    }
    if(goodSum > evilSum) return "Battle Result: Good triumphs over Evil"
    if(evilSum > goodSum) return "Battle Result: Evil eradicates all trace of Good"
